    ...
    <v-dialog v-model="model" persistent width="auto">
        <v-form @submit.prevent="sharingToUsers()">
            <v-card>
                ...
                <p v-if="errors.permission" class="text-center pa-1 text-red">
                    {{ errors.permission }}
                </p>
                <div class="d-flex">
                    <v-checkbox
                ...
                <v-card-actions class="margin-center d-flex justify-center">
                    <v-btn class="bg-white" @click="closeShareDialog" size="x-large">
                        {{ $t('global.close') }}
                    </v-btn>
                    <v-btn type="submit" class="bg-orange" size="x-large">
                        {{ $t('userAccount.share') }}!
                    </v-btn>
                </v-card-actions>
            </v-card>
