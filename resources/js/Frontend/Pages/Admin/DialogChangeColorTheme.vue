    ...
    <v-dialog>
        <v-card>
            ...
            <div class="justify-center d-flex py-6">
                <v-color-picker v-model="picker" elevation="5"></v-color-picker>
            </div>
            <v-card-actions class="margin-center">
                <v-spacer></v-spacer>
                <v-btn class="bg-white" @click="close" size="x-large" variant="outlined">
                    {{ $t('global.close') }}
                </v-btn>
                <v-btn class="bg-red" size="x-large" @click="setTheme">
                    {{ $t('global.change') }}
                </v-btn>
            </v-card-actions>
        </v-card>
...
const setTheme = () => {
    Inertia.put(
        route('admintheme.color', { color: picker.value }),
        {},
        {
            onFinish: () => {
                close()
