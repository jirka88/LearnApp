    ...
    <component :is="DashboardLayout">
        <v-container class="pa-0">
            <div class="d-flex flex-column pa-5 ga-6">
                <Breadcrumbs
                    :items="[{ title: 'předměty', disabled: true }]"
                ></Breadcrumbs>
                <div
                ...
                <v-table class="text-left">
                    ...
                    <tbody v-else-if="loading">
                        <TableSkeleton
                            v-for="n in subjectsShow"
                            :countTd="$page.props.user.role.id === 4 ? 5 : 6"
                            :key="n.id"
                        >
                        </TableSkeleton>
                    </tbody>
                    <tbody v-else>
                        <tr>
                            ...
                            <td v-else class="text-center" colspan="5">
                                Předměty nebyly vytvořeny!
                            </td>
                        </tr>
                    </tbody>
...
const fetchData = () => {
    Inertia.get(
        route('subject.index'),
        { page: page.value, sort: filtrGlobal.value },
        {
            preserveState: true,
            onSuccess: (response) => {
...
const sorted = async (filtr) => {
    ...
    await axios
        .get(`/dashboard/manager/subjects/sort?sort=${filtr.sort},${filtr.id}`)
        .then((response) => {
