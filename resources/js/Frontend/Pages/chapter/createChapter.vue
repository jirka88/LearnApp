<template>
    <component :is="DashboardLayout">
        <div
            class="creatingChapter d-flex justify-center align-center primary-bg"
        >
            <v-container class="pa-8">
                <BackBtn :url="route('subject.show', slug)" />
                <form
                    class="pa-8 py-4 mt-4 w-100 d-flex flex-column elevation-20"
                    @submit.prevent="createChapter"
                >
                    ...
                    <v-no-ssr>
                        <QuillEditor
                            ...
                            content-type="html"
                            :style="
                                form.errors.contentChapter
                                    ? { border: '1px solid red !important' }
                                    : {}
                            "
                        />
                        <p class="text-red pt-1 subtitle-2">
                            {{ form.errors.contentChapter }}
                        </p>
                    </v-no-ssr>
                    <v-btn
                ...
                <Toastify
                    v-if="
                        isActiveToast &&
                        (form.errors.name || form.errors.message)
                    "
                    :text="
                        form.errors.name
                            ? form.errors.name + ' '
