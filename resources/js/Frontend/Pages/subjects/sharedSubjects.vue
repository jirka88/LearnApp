    ...
    <component :is="DashboardLayout">
        <v-container>
            <div class="d-flex flex-column py-5 ga-6">
                <Breadcrumbs
                    :items="[{ title: 'Sdílení', disabled: true }]"
                ></Breadcrumbs>
            </div>
                ...
                <thead>
                    ...
                    <tr v-if="subject.users.length > 0">
                        ...
                        <th class="font-weight-bold">
                            {{ $t('share.selected_permission') }}
                        </th>
                        <th class="font-weight-bold">Nastavení:</th>
                    </tr>
                </thead>
                <tbody v-for="user in subject.users" :key="user.id">
                    <tr class="pa-8">
                        <td width="5%">
                            <v-img
                                height="2em"
            ...
            <shareForm
                v-if="shareFormActive"
                :active="shareFormActive"
                :detail="usr"
                :permission="permissions"
                @close="shareFormActive = false"
            ></shareForm>
        </v-container>
    </component>
</template>
